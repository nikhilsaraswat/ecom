...
<div class="container-fluid">
  ...
  @include("../components/sidebar")
    ...
    <div class="border-bottom px-4 py-2 font-bold text-xl d-flex justify-content-between align-items-center" >
      ...
      <form method="POST" enctype="multipart/form-data" class="h-90 flex flex-column items-center space-y-2 p-3 bg-white" action="/admin/subcategoryedit/{{ $subcategorylistfromdatabase->id }}" >
  @csrf
  @method('PUT')
  <label for="name" class="form-label text-right mr-2 " >Category:</label>
  <div class="form-group mb-2 d-flex">
    <select name="category" value="{{ $subcategorylistfromdatabase->category_name }}" id="category" class="form-control px-3 py-2 rounded-md" required>
    ...
    <a href="{{ route('admincategorypanelcreation') }}" >
            <div class="btn btn-sm btn-primary" ><i class="fas fa-plus-circle" ></i> Create
</div>
        </a>
  </div>
  <div class="form-group mb-2 ">
    <label for="subcategory" class="form-label text-right mr-2 " >Sub Category:</label>
    <input type="text" name="subcategory" id="subcategory" value="{{ $subcategorylistfromdatabase-> subcategory }}" class="form-control px-3 py-2 rounded-md" required>
  </div>
  @error('subcategory')
  ...
  <div class="form-group mb-2">
    ...
    <img src="{{ asset('storage/images/' . $subcategorylistfromdatabase->image) }}" id="image-previous" class="img-thumbnail mt-2" style="max-width: 150px;" alt="previous">
</div>
