...
<div class="container-fluid">
  ...
  @include("../../components/sidebar")
    ...
    <div class="col-md-10" style="65vh">
      ...
      <div>
        <a href="{{ route('adminvariationproductpanel') }}" >
...
<div style="overflow-y: auto; height:75vh;">
  <div class="h-100">
    <form method="POST" enctype="multipart/form-data" class="h-90 flex flex-column items-center space-y-2 p-3 bg-white" action="{{ route('adminvariationproductpanelcreationpost') }}" >
      @csrf
      @method('Post')
      <div class="form-group mb-2 ">
        <label for="ProductName" class="form-label text-right mr-2 " >Product Name:</label>
        <select name="ProductName" id="ProductName" class="form-control px-3 py-2 rounded-md" required>
        <option disabled>-*-----*Select*------*-</option>
        @foreach($productList as $product=>$id)
      ...
      <div class="form-group mb-2 ">
        <label for="Variable" class="form-label text-right mr-2 " >Variable:</label>
        <select name="Variable" id="Variable" class="form-control px-3 py-2 rounded-md" required>
        <option value=""></option>
        @foreach($variableList as $value=>$attribute)
        @php
        $attribute=$attribute
        @endphp
      ...
      <div class="form-group mb-2 ">
        <label for="Valuevar" class="form-label text-right mr-2 " >Value:</label>
        <select name="Valuevar" id="Valuevar" class="form-control px-3 py-2 rounded-md " required>
        <option value=""></option>
        ...
        @foreach($value as $value)
            <option value="{{$attribute}}" class="{{$attribute}} displaynonevar d-none">{{$value }}</option>
        @endforeach
        @endforeach
    </select>
      </div>
...
$(document).ready(function() {
  ...
  $("#Variable").on("change",function(){
    $(".displaynonevar").addClass('d-none');
...
<script>
  // Sample data representing the variable list
  var variableList = ["Option1", "Option2", "Option3", "Option4"];

  // Function to create the form group with select element
  function createFormGroup() {
      ...
      label.className = 'form-label text-right mr-2';
      label.textContent = 'Variable:';

      var select = document.createElement('select');
      select.name = 'Variable';
      select.id = 'Variable';
      select.className = 'form-control px-3 py-2 rounded-md';
      ...
      variableList.forEach(function(attribute) {
  ...
  formContainer.appendChild(createFormGroup());
</script>
    </div>
  </div>
</div>
